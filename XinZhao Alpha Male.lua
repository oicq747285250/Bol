...
XinZhao Alpha Male by oicq747285250
Last Update:v 1.0
2015/1/15
...
local UPDATE_PATH = "/oicq747285250/Bol/master/XinZhao Alpha Male.lua".."?rand="..math.random(1,10000)
local UPDATE_FILE_PATH = SCRIPT_PATH..GetCurrentEnv().FILE_NAME
local UPDATE_URL = "https://"..UPDATE_HOST..UPDATE_PATH
function AutoupdaterMsg(msg) print("<font color=\"#FF0000\"><b>XinZhao Alpha Male:</b></font> <font color=\"#FFFFFF\">"..msg..".</font>") end
if _G.AUTOUPDATE then
	local ServerData = GetWebResult(UPDATE_HOST, "/oicq747285250/Bol/master/XinZhao Alpha Male.version")
	if ServerData then
...
local ts
local eRange = 600 -- Range E

function OnLoad()
		lastAttack = 0
		lastAttackCD = 0
		lastWindUpTime = 0
		PrintChat("<font color=\"#00ff00\">XinZhao Alpha Male by oicq747285250</font>")
	
		
		Config = scriptConfig("XinZhao Alpha Male "..version, "XinZhao Alpha Male "..version)
		Config:addSubMenu("[XinZhao Alpha Male]: Combo Settings", "combocfg")
		Config.combocfg:addParam("autoQWE", "AutoQWE while combo pressing", SCRIPT_PARAM_ONOFF, true)
...
function OnDraw()
	if(Config.drange) then
		...
		DrawCircle(myHero.x, myHero.y, myHero.z, eRange, 0x33CC33)
		end
	end
	test()
...
if (Config.combocfg.combokey) then
	...
	if(Config.combocfg.autoQWE) then
		if (ts.target ~= nil) then
			if (myHero:CanUseSpell(_W and _Q and _E) == READY) then
